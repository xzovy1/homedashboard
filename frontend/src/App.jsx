...
function App() {
  ...
  const colorSchemes = {
    sunrise:
      "linear-gradient(140deg, rgba(255, 197, 167, 1) 0%, rgb(255 193 192) 50%, rgb(255 248 184) 100%)",
    middayClear:
      "inear-gradient(140deg, #82C3FF 0%, #BFE2FF 50%, #E0F4FF 100%)",
    goldenHour:
  ...
  const [sensorData, setSensorData] = useState({
    office: defaultSensor,
    bedroom: defaultSensor,
    livingRoom: defaultSensor,
  });
  ...
  useEffect(() => {
    ...
    const secondaryComponentLS = JSON.parse(
      localStorage.getItem("secondaryComponent"),
    );
    secondaryComponentLS ? setWidgetComponentName(secondaryComponentLS) : null;
  }, [widgetBarStatus]);
  ...
  return (
    ...
    >
      <WidgetBar
        ...
        setWidgetComponentName={setWidgetComponentName}
      />
      <Hero>
        <FocusedComponent />
        <Calendar />
