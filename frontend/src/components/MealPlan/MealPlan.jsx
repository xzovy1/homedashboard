...
import { faDeleteLeft, faPlus, faPen, faCheck, faXmark } from "@fortawesome/free-solid-svg-icons";
    ...
    return (
        <form action={addMeal} className={heroClasses.header}>
            <div className={mealClasses.newMeal}>
                ...
                <input type="text" name="name" id="mealName" required  autoComplete="off"/>
            </div>
...
const Meals = ({ meals, setMeals }) => {
    ...
    const renameMeal = async (id, name) => {
        await fetch(`${URL}/${id}`, {
            ...
            .then(updated => setMeals(meals.map(m => m.id === id ? updated : m)));
    };
    ...
    if (meals.length > 0) {
        return (
            <div className={heroClasses.body}>
                {meals.map(meal => (
                    <MealItem key={meal.id} meal={meal} removeMeal={removeMeal} renameMeal={renameMeal} />
                ))}
            </div>
        );
    }
    ...
    return (
        <div className={mealClasses.mealItem}>
            ...
            <div className={mealClasses.actions}>
                {editing ? (
                    <>
                        <FontAwesomeIcon icon={faCheck} onClick={handleConfirm} className={mealClasses.fontAwesome} />
                        <FontAwesomeIcon icon={faXmark} onClick={handleCancel} className={mealClasses.fontAwesome} />
                    </>
                ) : (
                    <>
                        <FontAwesomeIcon icon={faPen} onClick={() => setEditing(true)} className={mealClasses.fontAwesome} />
                        <FontAwesomeIcon icon={faDeleteLeft} onClick={() => removeMeal(meal.id)} className={mealClasses.fontAwesome} />
                    </>
                )}
            </div>
        </div>
