...
export const MealPlanWidget = ({ setWidgetComponentName }) => {
    ...
    return (
        <div className={`${widgetStyles.widget} ${widgetStyles.box}`} onClick={() => focusComponent(setWidgetComponentName, 'Meal Plan')}>
...
const Meals = ({ meals }) => {
    if (meals.length > 3) {
        return <div><div>Total: {meals.length}</div></div>;
    }
    return (
        meals.length > 0
            ? <ul className={widgetStyles.list}>{meals.map(meal => <li style={{ fontSize: "smaller" }} key={meal.id}>{meal.name}</li>)}</ul>
